...
async function getManualNewVersion(latestTag) {
    console.log(`Latest version is: ${latestTag}`);
    while (true) { // Loop until valid input is received
        const newVersionInput = await new Promise((resolve) => {
...
async function getVersionWithPrefix(versionWithoutPrefix) {
    while (true) {
...
async function getAutomatedNewVersion(latestTag) {
...
async function getNewVersion(latestTag, options) {
    if (options.handly) {
...
async function confirmTagging(newVersion, latestTag) {
    let branch = '';
    let confirmed = false;
    while (!confirmed) {
...
async function handleTagging(newVersion, branch) {
    try {
...
async function getGitLabToken(host) {
    try {
        let config = {};
...
async function getGitLabProjectAndPipelineIds() {
    try {
...
async function getPipelineStatus(projectId, pipelineId, gitlabAddress) {
    try {
        const agent = new https.Agent({
            rejectUnauthorized: false // This tells Axios to trust self-signed certificates
        });
        const response = await axios.get(`https://${gitlabAddress}/api/v4/projects/${projectId}/pipelines/${pipelineId}`, {
...
async function displayPipelineStatus(projectId, pipelineId, gitlabAddress) {
...
export async function tagProject(options) {
    try {
